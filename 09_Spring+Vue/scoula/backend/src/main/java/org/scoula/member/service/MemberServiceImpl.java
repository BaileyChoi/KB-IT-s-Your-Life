...
public class MemberServiceImpl implements MemberService {
    ...
    private void saveAvatar(MultipartFile avatar, String username) {
        ...
        if (avatar != null && !avatar.isEmpty()) {
            File dest = new File("C:/KB_Bingbong/09_Spring+Vue/upload/avatar", username + ".png");
            try {
    ...
    public void changePassword(ChangePasswordDTO changePassword) {
        MemberVO vo = mapper.get(changePassword.getUsername());
        if (!passwordEncoder.matches(changePassword.getOldPassword(), vo.getPassword())) { // 비밀번호 일치 확인
            throw new PasswordMissmatchException();
        }
