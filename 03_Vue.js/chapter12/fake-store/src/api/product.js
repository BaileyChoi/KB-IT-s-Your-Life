...
export const post = async (target, params) => {
	try {
		const response = await axios.post(BASE + `${target}`, params);
		if (response.status === 200) {
			console.log(response);
			return response;
		}
...
export const put = async (target, params) => {
	try {
		const response = await axios.put(BASE + `${target}`, params);
		if (response.status === 200) {
			console.log(response);
			return response;
		}
